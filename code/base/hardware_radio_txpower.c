...
{
   log_line("Setting radio interface %d RTL8814AU raw tx power to %d using iw dev...", iCardIndex+1, iTxPower);
   if ( (iTxPower < 1) || (iTxPower > MAX_TX_POWER) )
      iTxPower = DEFAULT_RADIO_TX_POWER;
   ...
   for( int i=0; i<hardware_get_radio_interfaces_count(); i++ )
   {
      ...
      if ( NULL == pRadioHWInfo )
         continue;
      if ( (hardware_radio_driver_is_rtl8814au_card(pRadioHWInfo->iRadioDriver)) ||
           (pRadioHWInfo->iRadioType == RADIO_TYPE_RALINK) )
      {
         sprintf(szComm, "iw dev %s set txpower fixed %d", pRadioHWInfo->szName, -100*iTxPower);
         hw_execute_bash_command(szComm, NULL);
   ...
   log_line("Radio interface %d RTL8814AU raw tx power changed to: %d", iCardIndex+1, iTxPower);
}
...
{
  log_line("Setting radio interface %d RTL8852BE raw tx power to: %d", iCardIndex+1, iTxPower);
   if ( (iTxPower < 1) || (iTxPower > MAX_TX_POWER) )
      iTxPower = DEFAULT_RADIO_TX_POWER;
   ...
   for( int i=0; i<hardware_get_radio_interfaces_count(); i++ )
   {
      ...
      if ( NULL == pRadioHWInfo )
         continue;
      if ( hardware_radio_driver_is_rtl8852be_card(pRadioHWInfo->iRadioDriver) )
      {
         sprintf(szComm, "iw dev %s set txpower fixed %d", pRadioHWInfo->szName, iTxPower*40);
         hw_execute_bash_command(szComm, NULL);
      }
   ...
   log_line("Radio interface %d RTL8852BE raw tx power changed to: %d", iCardIndex+1, iTxPower);
}
